getInput:
	...
	addi $sp, $sp, -4
	sw $ra, 0($sp)
	jal allCapsBegin  #Convert user's string to capital letters. Stores the current $ra, since this is a jal
	lw $ra, 0($sp)
	addi $sp, $sp, 4
checkCenterLetter:
	lb $t5, CenterLetter #Load center letter
	li $t0, 0 #$t0 is counter for presence of center letter
	li $t1, 0 #$t1 is a counter for scanning string
centerLetterLoop:
	add $t2, $s0, $t1
	lb $t2, ($t2)
	beq $t2, $t5, goodLetter #If the character in $t2 is the center letter, increment the counter
	beq $t2, 10, finalCheck  #Newline = end of string
	addi $t1, $t1, 1
	j centerLetterLoop
goodLetter:
	addi $t0, $t0, 1
finalCheck:
	beq $t0, $zero, wrongUserInput #If its 0, the center letter was not used.
	li $v0, 4
	la $a0, userInput
	syscall
	#jr $ra
...
compareFoundWords: 
  ...
  lw $t2, 2($t1)                             	#t2 points to base address of a string
comparisonLoop2:
...
addWord:
  ...
  la $t8, gottenWords($t0)                    # Store this address in the wordArray
  sw $t7, 2($t8)			#Mysteriously, gottenWords is not word-aligned. I don't know why, this fixes it though.
...
rightUserInput:
	...
	la $t0, currentScore	# load address of score
	lw $t1, ($t0)
	add $t1, $t1, 5		# add to current score ( i put 5 points, could be anything )
	sw $t1, ($t0)		# store current score
	j jumpPrintBox
