...
importFile:
	...
	add $a0, $t9, $zero	# file descriptor back to $a0
	...
	syscall			# file descriptor still in $a0
	jr $ra			# return
...
getNineLetter:
	...
	######### setting up for finding the 9 letter word
	######### getting a random number between 0 and the length of the list
	...
	add $s6, $v0, 0		# store random number in $s6
	########### updated to find the 9 letter word faster
getNineLetLoop:
	...
	addi $s6, $s6, -1	# subtract 1 from pos to move backwards until a 9 letter word is found
...
getNineLetReturnLoop:		# saves the 9 letter word to wordInBox
	beq $t0, 9, fillCorrectArray	# when counter hits 9, jump to filling the array
